...
class DrawMode(Mode):
    ...
    def redrawAll(self,canvas):
        ...
        x1 = self.x + 40
        y1 = self.y + self.r
        canvas.create_rectangle(x0,y0,x1,y1, fill = "red", onClick = self.onClick,width = 0)
        canvas.create_text((x1 - 40), (y1 - 20), text = "Play")
        x2 = x1 + 40
        x3 = x2 + 80
        canvas.create_rectangle(x2,y0,x3,y1, fill = "light blue", onClick = self.randomClicked,width = 0)
        canvas.create_text((x3 - 40), (y1 - 20), text = "Random")
        x4 = x3 + 40
        x5 = x4 + 80
        y3 = self.y + self.r
        canvas.create_rectangle(x4,y0,x5,y1, fill = "orange", onClick = self.saveClicked,width = 0)
...
class Enemy(object):
    ...
    def move(self):
        ...
        if self.col + 3 < self.squareCol:
            if self.mapList[math.floor(self.row)][math.floor(self.col) + 1] == 1:
                ...
                if self.mapList[math.floor(self.row) - 1][math.floor(self.col) + 1] == 1:
                    self.row -= 0.25
        elif self.col + 3 > self.squareCol:
            if self.mapList[math.floor(self.row)][math.floor(self.col) - 1] == 1:
                self.col -= 0.25
            elif self.row < self.squareRow:
                if self.mapList[math.floor(self.row) + 1][math.floor(self.col) - 1] == 1:
                ...
                if self.mapList[math.floor(self.row) - 1][math.floor(self.col) - 1] == 1:
                    self.row -= 0.25
        elif self.col + 3 == self.squareCol and self.row == self.squareRow:
            self.app.die = True
    ...
    def render(self,canvas):
        for i in range(len(self.baseGeometry.arcList)):
            row = i // 31
            col = i % 31
            if math.floor(self.row) == row and math.floor(self.col) == col:
                self.arc = self.baseGeometry.arcList[i]
                x0 = self.arc.cX - self.arc.r - 7.5
                y0 = self.arc.cY - self.arc.r - 7.5
                x1 = self.arc.cX + self.arc.r + 7.5
                y1 = self.arc.cY + self.arc.r + 7.5
                canvas.create_arc(x0,y0,x1,y1,style = basic_graphics.ARC,width = 15, start = 90 - self.arc.degree * self.arc.i + 2.5, extent = -self.arc.degree + 5, outline = "red")
...
class GameMode(Mode):
    ...
    def redrawAll(self,canvas):
        ...
        if self.mapMode:
            ...
            canvas.create_text((x3 - 30), (y1 - 20), text = "solution")
            for arc in self.arcList2:
                arc.render(canvas)
            x0 = 450 - 50
            y0 = 450 - 50
...
class GameLevel(GameMode):
    ...
    def timerFired(self):
        ...
        if self.startRotate == True:
            for arc in self.arcList:
                arc.rotate()
        if self.timerCalls % 20 == 0:
            self.enemy.move()
    
    def redrawAll(self,canvas):
        super().redrawAll(canvas)
        self.enemy.render(canvas)
...
class unitArcList(object):
    ...
    def positions(self):
        ...
        if self.app.square.y <= self.app.height / 2 - 37.5:
            self.currRow = (self.app.square.y - 75) // 25 + 3
            for i in range(len(self.arcList)):
                row = i // 32
                col = i % 32
...
class MapGenerator(Mode): 
    ...
    def redrawAll(self,canvas):
        ...
        x4 = x3 + 80
        x5 = x4 + 40
        canvas.create_rectangle(x0,y0,x1,y1, fill = "light blue", onClick = self.randomClick,width = 0)
        canvas.create_text((x1 - 30), (y1 - 20), text = "random")
        canvas.create_rectangle(x2,y0,x3,y1, fill = "orange", onClick = self.startGameClick, width = 0)
...
# algorithmic concepts and ideas for this class from <mazes for programmers>  see top
	...
	def findPath(self, destination):
		...
		def dfs(path):
			...
			for drow,dcol in [[0, 1], [0, -1], [1, 0], [-1, 0]]:
				row, col = path[-1][0] + drow, path[-1][1] + dcol
